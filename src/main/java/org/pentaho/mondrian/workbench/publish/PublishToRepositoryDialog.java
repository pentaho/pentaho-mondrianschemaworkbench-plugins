...
public class PublishToRepositoryDialog extends JDialog implements SolutionRepositorySelectionCallback {
  ...
  public PublishToRepositoryDialog(
    ...
    this.parent = parent;
    init(showFoldersOnly, webPublishURL, publishLocation, userid, password, filters, schemaName, schemaFileName, jndiName, enableXmla);
  }
  ...
  public void init(
    ...
    final IndeterminateProgressDialog progressDialog = 
        new IndeterminateProgressDialog((JFrame)parent, "Progress Dialog", "Connecting to Repository...");
    WindowUtils.setLocationRelativeTo(progressDialog, getParent());
    ...
    Thread worker = new Thread() {
        public void run() {
            try {
                PublishToRepositoryDialog.this.repositoryBrowser = 
                    new SolutionRepositoryTableView(showFoldersOnly, webPublishURL, 
                            filters, userid, password, PublishToRepositoryDialog.this);
            } catch (Exception e) {
                exceptions.add(e);
            } finally {
    ...
    if (this.repositoryBrowser == null && exceptions.size() == 0) {
    ...
    } else if (repositoryBrowser == null) {
        if (worker.isAlive()) {
